...
# important: on Business/Enterprise accounts an admin policy override
# can leave the http value intact while flipping mode back to system,
# which makes the proxy inert. A value-only grep would false-positive
# in that case.
#
...
# a pull blocks for the full TCP SYN timeout (~75s on macOS) per
# attempt, turning the polling loop into a multi-minute hang. The
# trainee will run docker pull themselves to observe the symptom; the
# break script must not.
# ------------------------------------------------------------------
echo ""
...
if [ "$PROXY_ACTIVE" -eq 0 ]; then
    ...
    echo "  Possible cause: admin policy override is reverting mode to"
    echo "  'system' on this account. Verify manually:"
