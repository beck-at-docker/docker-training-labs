#!/bin/bash
# break_dns.sh - Breaks Docker daemon DNS resolution by injecting iptables DROP
# rules for port 53 inside the Docker Desktop VM via nsenter.
#
# The daemon process runs inside the VM and uses the VM's network stack for its
# own DNS lookups (e.g. registry resolution during docker pull). Dropping port
# 53 traffic at the VM's OUTPUT chain prevents the daemon from resolving any
# external hostnames, producing errors like:
#
#   lookup http.docker.internal on 192.168.65.7:53:
...
# Desktop as a last resort (partial credit).
