...
test_fixed_state() {
    ...
    run_test "Container can reach internet (IP)" \
        "timeout 10 docker run --rm alpine:latest ping -c 3 8.8.8.8 > /dev/null"
    
    run_test "Container can reach internet (hostname)" \
        "timeout 10 docker run --rm alpine:latest ping -c 3 google.com > /dev/null"
    ...
    run_test "Container-to-container communication restored" \
        "timeout 10 docker run --rm alpine:latest ping -c 2 $web_ip > /dev/null"
